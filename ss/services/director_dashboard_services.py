...
class DirectorDashboardService:
    ...
    def __init__(self, filters: dict | None = None):
        ...
        filters pot contenir:
          ...
          El que es passara sera algo aixi:
          filters = {
            "period": "week",
            "content_type": "film",
            "platform_id": 2
    ...
    def _get_visualizations_by_genre(self) -> dict:
        ...
        qs = (
            ...
            .values("genre__name")      
            .annotate(total=Count("id"))            #Equival a un group by
            .order_by("-total")
        )
        return {
    ...
    def _get_visualizations_by_type(self) -> dict:
        ...
        film_count  = self._visualizations_qs.filter(content_id__in=film_ids).count()  #Aqui es on fa la consulta realment
    ...
    def _get_top_genres(self) -> dict:    #serveix nms per la
        """
    ...
    def _build_base_queryset(self):
        ...
        """
        qs = Visualization.objects.all()  #Es crea un objecte del tipus QuerySet, internament com SELECT * FROM visualization
        ...
        if content_type == "film":
            film_ids = Film.objects.values_list("content_ptr_id", flat=True)  #Es un altre QuerySet lazy que fa SELECT content_ptr_id FROM film
